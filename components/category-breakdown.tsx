...
export function CategoryBreakdown({ transactions }: CategoryBreakdownProps) {
  const summaries = summarizeBy(transactions, "category");

  return (
    <Card className="space-y-6">
      <div className="space-y-1.5">
        <h2 className="text-xl font-semibold text-[var(--foreground)]">Category breakdown</h2>
        <p className="text-sm text-[var(--muted)]">See which kinds of work are shaping this selected period.</p>
      </div>

      {summaries.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-sm text-slate-500">
          No category totals to show for this filter yet.
        </div>
      ) : (
